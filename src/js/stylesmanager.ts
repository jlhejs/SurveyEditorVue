...
export class StylesManager{
  ...
  static createSheet(styleSheetId: string) {
    ...
    return <any>style.sheet;
  }

  public static currentTheme = Vue.observable("bootstrap");

  public static applyTheme(themeName: string = "default",themeSelector: string = ".svd_container") {
    var self:any=this
    self.currentTheme=themeName;

    self.applySurveyTheme(themeName);

    let sheet = self.findSheet(themeName + themeSelector);
    if (!sheet) {
      sheet = self.createSheet(themeName + themeSelector);
      let theme =
      self.ThemeColors[themeName] ||
      self.ThemeColors["default"];

      let ThemeCss;
      if (["bootstrap", "bootstrapmaterial"].indexOf(themeName) !== -1) {
        ThemeCss = self[themeName + "ThemeCss"];
      } else {
  ...
  public static applySurveyTheme(themeName:string) {
    if (["bootstrap", "bootstrapmaterial", "modern"].indexOf(themeName) !== -1) {
      var s:any=Survey.Survey
      s.cssType = themeName;
    } else {
      var s:any=Survey.Survey
      s.cssType = "default";
